...
class Bullet{
  ...
  move(){
    ...
    //this란?
    //거푸집(class)로부터 태어난 인스턴스가 자기 자신을 가리킬수 있는 변수
    //아래의 코드에서, move()내에서 인스터스인 현재 총알이 자기자신을
    //가리켜야 하므로 this를 사용함

    //현재 총알의 인스턴스와 게임에 존재하는 모든 적군과의 충돌검사 후, 
    //충돌이면 (총알을 화면에서제거+ 총알을 배열에서제거)
    //충돌이면 (적군을 화면에서제거+ 적군을 배열에서제거)
    for(let i=0; i<enemyArray.length;i++){

      let result = collisionCheck(this, enemyArray[i]); //충돌여부를 판단하기
      if(result){ //충돌 감지

        score +=enemyArray[i].score; // 전역변수 score에 10씩 누적
    ...
    //화면의 한계점을 넘어설때는, 총알을 제거(화면에서제거+ 배열에서제거)
    if(this.x >=1280){
      //1.화면에서 제거
      wrapper.removeChild(this.div);
      //2. 배열에서도 제거
