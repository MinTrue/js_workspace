/*
우리는 이미 적군 클래스를 가지고 있음에도, 아래의 클래스를 별도로 만드는 이유는
부모로 물려받는 기능이 현재 상항에 맞지 않을 경우 업그레이드 할 필요가 있다.
즉, 부모가 물려준 메서드를 동일한 이름으로 재정의하는 기법을 오버라이딩(OverRiding)
이떄, 메서드명이 부모와 자식 둘다 보유하고 있으므로, 호출시엔 자식의
